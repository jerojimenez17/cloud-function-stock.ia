...
/**
 * Cloud Function HTTP handler for getting the last voucher number
 */
export const getLastVoucherHandler = onRequest(async (request, response) => {
  const allowedOrigins = [
    "https://prueba-demo-app.vercel.app",
    "http://localhost:3000",
  ];
  ...
  try {
    const {getLastVoucher} = await import("./functions/getLastVoucher.js");
    const result = await getLastVoucher(request.body);
    ...
    if (result.success) {
      ...
      console.error("Validation error:", JSON.stringify(result.error, null, 2));
      const errorMsg = result.error?.message ||
        JSON.stringify(result.error) || "Error obteniendo último comprobante";
      response.status(400).send({error: errorMsg, details: result.error});
    }
  } catch (error) {
    logger.error("Internal Server Error detailed:", {
    ...
    response.status(500).send({
      error: "Error obteniendo último comprobante",
      details: (error as Error).message,
    });
  }
});
